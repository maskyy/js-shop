...
type Filter = FilterEstate | FilterCar | FilterLaptop | FilterCamera;
 ...
 * @returns {ServerData} JSON из ссылки
 */
async function fetchData(url: string): Promise<ServerData> {
 ...
 */
function updateItem(selector: string, text: string): void {
 ...
 */
function updateMap({coordinates}: {coordinates: L.LatLngExpression}, address: string): void {
...
const DATA_URL: string = 'https://main-shop-fake-server.herokuapp.com/db';
...
const products: Array<Product> = (await fetchData(DATA_URL)).products;
